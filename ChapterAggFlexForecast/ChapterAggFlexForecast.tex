...
\begin{algorithm}
	...
	\KwResult{$F \gets$ empty list of forecast values for each signal [kWh or share], indexed by $F^{l}_{t,m}$}
	
\For{all $m \in M\{1,12\}$}{	
	 \For{all $t \in T$} {
	 \For{all $l \in L$}{
	 $F^{l}_{t,m} = \frac {1}{S} \ \mathlarger{\mathlarger{\sum}}_{i=1}^{S} \ \ \mathlarger{\mathlarger{x}}_{t = T,m=M} $\\
	 $F^{l}_{t,m} = F^{l}_{t} \qquad \forall m \in M[1,12]$
	}
	}
	}
