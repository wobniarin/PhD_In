...
The dynamics of the power system are changing towards a new model where large generators on the HV side of the grid are being replaced by smaller generation units placed at the MV and LV side of the grid. This increase of penetration by DG will have a significant impact on how the distribution network operates. At the distribution level, technically speaking, distributed generation will make arise some new challenges and, in some cases, problems already solved will rise again. 

This chapter aims to develop an optimization tool for calculating the value and location of the flexibility request that a DSO needs for operating the distribution network and avoid or mitigate network congestions, focused on problems $(i)$ and $(ii)$. This is objective $(iv)$ of the thesis research, according to Figure \ref{fig:chapter_obj_iv}. 
...
The most common problems caused by the high penetration of distributed and renewable generation can be classified into four main categories as done in XXXX. Figure XXX provides an overview of the location of these potential problems in a arbitrary distribution network:  
...
\subsection{Risk of overload/overcurrents}
...
\subsection{Risk of overvoltage}
