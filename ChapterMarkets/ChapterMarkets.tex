...
\begin{table}[]
\centering
\caption{Local Markets Definition Review}
\label{table22Local}
\resizebox{\textwidth}{!}{%
...
{[}26{]} &
  2010 &
  Local Energy Market: Highly flexible market platform, to coordinate self-interested energy agents representing power suppliers, customers, and prosumers. \\ The energy agents implement a generic bidding strategy that can be governed by bidding policies. \\
{[}27{]} &
  2010 &
  Trading agent for Smart Grids: New electricity market mechanism for self-interested agents, to create a decentralized autonomous system. The aim is to manage the self-interested actions of the participants while guaranteeing a high level of surplus and ensuring that transmission lines are never overloaded. \\
{[}28{]} &
  2011 &
  Demand Response eXchange (DRX): Competitive trading platform, used to sell/buy demand response as a commodity between buyers and sellers. (Local Market for Flexibility services) \\
{[}29{]} &
  2011 &
  Smart City Energy Marketplace: Neighborhood or district-wide marketplaces within a smart grid where prosumers may interact portions of their prosumed energy. \\
{[}30{]} &
  2013 &
  DSO-Market on flexibility services (FLECH): Marketplace where the flexible DER of the consumers can be mobilized by aggregators for providing flexibility products to the DSO or TSO. \\
{[}31{]} &
  2012 &
  Local Energy Market: Energy market at smart neighborhood district level with the primary goal of facilitating and managing the electricity trading between the citizens of this smart neighborhood. Additionally, the implied aim is also to use it for market-driven demand response. \\
{[}32{]} &
  2012 &
  Consumer driven market (local market): Wholesale market structures, where consumer access to these markets. Energy marketplace to enhance small consumer and local generation participation in the distribution constrained power. \\
{[}33{]} &
  2013 &
  Local Reserve Energy Market: Auction mechanism that aims to enable regionally or virtually restricted trading of ancillary services. \\
{[}34{]} &
  2014 &
  Local Electricity Market: Geographic area where consumption and production can be metered, there is no transmission capacity restriction between the Market Balanced Areas and for which there is one balance responsible party (BRP), and, thus, one price for the imbalance. \\
{[}35{]} &
  2014 &
  Local Market: Energy exchange market in a local way so as to manage local network congestions. \\
{[}36{]} &
  2015 &
  Local Electricity Market: Geographic area where consumption and production can be metered, there is no transmission capacity restriction between the Market Balanced Areas and for which there is one balance responsible party (BRP), and, thus, one price for the imbalance. \\
{[}37{]} &
  2016 &
  Local Electricity trading market: Trading area which allows not only local users but also suppliers to trade excess electricity generated by RESs. In addition, a set of functional requirements and potential interactions among different entities are provided. \\
{[}38{]} &
  2016 &
  Local Flexibility Market: Long or short-term trading actions for flexibility in a specific geographical location, voltage level and system operator (DSO and TSO), given by grid condition or balancing needs, where participants in a relevant market can be aggregated to provide flexibility services. \\
{[}39{]} &
  2016 &
  Local Power Market: Trading arena, grounded on a local community and includes different types of prosumers, consumers, producers and storage facilities. It engages community members and those sharing the interest of the community in an array of commercial activities that serve to create a better and more sustainable energy experience for all parties involved.  It supports energy related exchanges. \\
{[}39{]} &
  2016 &
  Local Energy Market: Trading platform which aims to schedule the local resources at minimum cost to get an optimal balancing between local demand, local supply and grid exchange. \\
{[}39{]} &
  2016 &
  Local Flexibility Market: Trading platform to adjust the energy resources to correct forecasting errors or to increase the participants' profits in balancing markets. \\
{[}39{]} &
  2016 &
  Local Market for Other Services: Trading platform for other services such as maintenance, failure detection and technical user support. \\
{[}40{]} &
  2016 &
  Local AS Market: Real time energy market to consider the activation of balancing and congestion management services, both in Transmission and distribution ancillary services. The DSO operates a local market to solve distribution grid problems and then aggregates and offers the remaining flexibility bids to the TSO Markets.  Market for Ancillary Services for DSO and TSO. \\
{[}40{]} &
  2016 &
  Common TSO-DSO AS Market Model: Real time energy market to consider the activation of balancing and congestion management services, both in Transmission and distribution ancillary services. TSO and DSO have the common objective to minimize the total costs needed to satisfy their respective services (AS for TSO and local services for DSO), by using a decentralized architecture, but with a dynamic integration of a local market operated by the DSO. \\
{[}41{]} &
  2016 &
  Local Flexibility Market: Decentralized implicit interaction framework for trading flexibility from prosumers. Consists of one market operator, one DSO, a number of energy suppliers, aggregators and balance responsible parties, which aim to exploit flexibility that is available on the demand side. Two mechanisms: ahead planning via markets and real-time dispatching \\
{[}42{]} &
  2016 &
  Local Flexibility Market: Marketplace which aim is to solve low voltage grid violations with regional flexibility. \\
{[}23{]} &
  2017 &
  Local Energy Market: Marketplace on which prosumers and consumers of a community are able to trade electricity directly amongst each other at variable prices. The aim is to facilitate a local balance of energy supply and demand, in a decentralized approach \\
{[}45{]} &
  2017 &
  Local Electricity Market: Market setup to create economic incentives for the expansion of generation capacity close to load centers. The objective is to solve redispatch locally. \\
{[}44{]} &
  2017 &
  Local Energy Market: Geographically constrained market mechanisms with distinct pricing mechanisms between interconnected agents i (i.e. producers, prosumers and consumers). The agents have an energy generation git and demand dit per time slot t ? T. The market mechanism allows for trading energy between the agents. \\
{[}46{]} &
  2018 &
  Local Flexibility Market: Electricity trading platform to sell and buy flexibility in the Local Energy Community (LEC). In order to run this market, local traders need the Smart Energy Service Provider (SESP) Platform. It acts as local market facilitator for the LEC and as an aggregator for wholesale market agents. \\
{[}47{]} &
  2018 &
  Local Renewable Energy Balancing Market: Auction mechanism in charge of the efficient matching of households offers to buy and sell renewable energy, based on a Blockchain transactive platform. \\
{[}48{]} &
  2018 &
  Local Market Parties: Small parties considered as aggregators or flexibility services providers to DSO and TSO. \\
{[}49{]} &
  2018 &
  Local Flexibility Market: Electricity trading platform to sell and buy flexibility in geographically limited areas like neighbourhoods and small towns. The SESP is the local market platform provider and community aggregator (AGR). At the same time, the SESP is a BRP from the regulatory point of view because it bids in wholesale markets. \\
{[}50{]} &
  2018 &
  Energy Collectives: Community of prosumers that operates in a collaborative manner, optimizing usage of resources. The market framework in that where the collective members can trade their lack or excess of energy. All prosumers are in charge of optimizing their assets individually.  Optimality is achieved as prosumers are coordinated by a non-profit virtual node, named community manager. \\
{[}8{]} &
  2018 &
  Full P2P Market: Decentralized electricity market that implies that each agent (i.e. producers, consumers and prosumers) directly interacts with the other agents without intermediary entities like a retailer or market operator. \\
{[}8{]} &
  2018 &
  Community-based market: Decentralized electricity market structured with a third entity (community manager), to manage transactions among agents within the community and additionally can intermediates between the community and other communities and existing markets. \\
{[}8{]} &
  2018 &
  Hybrid P2P market: Combination of Full P2P Market and Community-based market, ending up with different layers for trading energy. \\ \bottomrule
\end{tabular}%
}
\end{table}
...
\subsection{Involved agents and stakeholders}
...
\begin{figure}[]
	\centering
	\includegraphics[width=0.4\columnwidth ]{ChapterIntro/Figures/Figure2.8.jpg}
	   %\vspace*{-8cm}
...
\begin{figure}[]
	\centering
	\includegraphics[width=0.4\columnwidth ]{ChapterIntro/Figures/Figure2.11.jpg}
	   %\vspace*{-8cm}
